...
# sameersbn/invoiceplane:1.4.6-3
...
```bash
docker pull sameersbn/invoiceplane:1.4.6-3
```
...
docker run --name invoiceplane -itd --restart=always \
  ...
  sameersbn/invoiceplane:1.4.6-3 app:invoiceplane
```
...
docker run --name invoiceplane-nginx -itd --restart=always \
  ...
  --publish 10080:80 \
  sameersbn/invoiceplane:1.4.6-3 app:nginx
```
...
docker run --name invoiceplane -it --rm [OPTIONS] \
  sameersbn/invoiceplane:1.4.6-3 app:backup:create
```
...
```bash
docker run --name invoiceplane -it --rm [OPTIONS] \
  sameersbn/invoiceplane:1.4.6-3 app:backup:restore
```
...
```bash
docker run --name invoiceplane -it --rm [OPTIONS] \
  sameersbn/invoiceplane:1.4.6-3 app:backup:restore BACKUP=1417624827_invoiceplane_backup.tar
```
