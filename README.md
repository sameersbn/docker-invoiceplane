...
```bash
docker pull sameersbn/invoiceplane:latest
```
...
docker run --name invoiceplane -itd --restart=always \
  ...
  sameersbn/invoiceplane:latest app:invoiceplane
```
...
docker run --name invoiceplane-nginx -itd --restart=always \
  ...
  --publish 10080:80 \
  sameersbn/invoiceplane:latest app:nginx
```
...
docker run --name invoiceplane -it --rm [OPTIONS] \
  sameersbn/invoiceplane:latest app:backup:create
```
...
```bash
docker run --name invoiceplane -it --rm [OPTIONS] \
  sameersbn/invoiceplane:latest app:backup:restore
```
...
```bash
docker run --name invoiceplane -it --rm [OPTIONS] \
  sameersbn/invoiceplane:latest app:backup:restore BACKUP=1417624827_invoiceplane_backup.tar
```
